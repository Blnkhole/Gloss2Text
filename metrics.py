...
#We use weighted WER (3,3,4) following original paper
WER_COST_DEL = 3
WER_COST_INS = 3
WER_COST_SUB = 4
...
def chrf(references, hypotheses):
    ...
    """
    return (
        chrf_metric.compute(predictions=hypotheses,references=[[i] for i in references])["score"] * 100
    )
...
def report_all(references, hypotheses):
    ...
    chrf_plus_res = chrf_metric.compute(
        predictions=hypotheses,
        references=[[i] for i in references],
        word_order=2
    )

    rouge_res = rouge(references, hypotheses)

    return {
        'bleu': bleu_res.score,
        'bleu_1': bleu_res.precisions[0],
        'bleu_2': bleu_res.precisions[1],
        'bleu_3': bleu_res.precisions[2],
        'bleu_4': bleu_res.precisions[3],
        'chrf': chrf_res['score'],
        'chrf++': chrf_plus_res['score'],
...
def token_accuracy(references, hypotheses, level="word"):
    ...
    for hyp, ref in zip(hypotheses, references):
        all_tokens += len(hyp.split(split_char))
...
def exact_match(refs, hyps):
    return sum(r == h for r, h in zip(refs, hyps)) / len(refs) * 100
